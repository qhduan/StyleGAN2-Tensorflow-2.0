...
def nImage(n):
    return np.random.uniform(0.0, 1.0, size=[n, im_size, im_size,
                                             1]).astype('float32')
...
# Blocks
def g_block(inp, istyle, inoise, fil, u=True, name=''):
    ...
    rgb_style = tf.keras.layers.Dense(
        fil,
        kernel_initializer=tf.keras.initializers.VarianceScaling(
            200 / out.shape[2]))(istyle)
    style = tf.keras.layers.Dense(inp.shape[-1],
                                  kernel_initializer='he_uniform')(istyle)
    ...
    # out = tf.keras.layers.Dense(fil, kernel_initializer='zeros')(out)
    out = tf.keras.layers.Add(name=f'g_block_{name}')([out, d])
    ...
    # out = tf.keras.layers.Dense(fil, kernel_initializer='zeros')(out)
    out = tf.keras.layers.Add(name=f'g_block_2_{name}')([out, d])
    out = tf.keras.layers.LeakyReLU(0.2)(out)

    return out, to_rgb(out, rgb_style)


def d_block(inp, fil, p=True, name=''):
    ...
    out = tf.keras.layers.Conv2D(filters=fil,
                                 kernel_size=3,
                                 padding='same',
    ...
    out = tf.keras.layers.Add(name=f'd_block_{name}')([res, out])
...
def to_rgb(inp, style):
    ...
    return tf.keras.layers.Lambda(upsample_to_size,
                                  output_shape=[None, im_size, im_size,
                                                None])(x)


def from_rgb(inp, conc=None):
    fil = int(im_size * 4 / inp.shape[2])
    z = tf.keras.layers.veragePooling2D()(inp)
...
class GAN(object):
    ...
    def discriminator(self):
        ...
        x = d_block(inp, 1 * cha, name='128')  # 128

        x = d_block(x, 2 * cha, name='64')  # 64

        x = d_block(x, 4 * cha, name='32')  # 32

        x = d_block(x, 6 * cha, name='16')  # 16

        x = d_block(x, 8 * cha, name='8')  # 8

        x = d_block(x, 16 * cha, name='4')  # 4

        x = d_block(x, 32 * cha, p=False, name='4_2')  # 4
    ...
    def generator(self):
        ...
        self.S.add(tf.keras.layers.Dense(512, input_shape=[latent_size]))
        self.S.add(tf.keras.layers.LeakyReLU(0.2))
        self.S.add(tf.keras.layers.Dense(512))
        self.S.add(tf.keras.layers.LeakyReLU(0.2))
        self.S.add(tf.keras.layers.Dense(512))
        self.S.add(tf.keras.layers.LeakyReLU(0.2))
        self.S.add(tf.keras.layers.Dense(512))
        ...
        for i in range(n_layers):
            inp_style.append(tf.keras.layers.Input([512]))

        inp_noise = tf.keras.layers.Input([im_size, im_size, 1])

        # Latent
        x = tf.keras.layers.Lambda(lambda x: x[:, :1] * 0 + 1)(inp_style[0])

        outs = []

        # Actual Model
        x = tf.keras.layers.Dense(4 * 4 * 4 * cha,
                                  activation='relu',
                                  kernel_initializer='random_normal')(x)
        x = tf.keras.layers.Reshape([4, 4, 4 * cha])(x)

        x, r = g_block(
            x, inp_style[0], inp_noise, 32 * cha, u=False, name='4')  # 4
        outs.append(r)

        x, r = g_block(x, inp_style[1], inp_noise, 16 * cha, name='8')  # 8
        outs.append(r)

        x, r = g_block(x, inp_style[2], inp_noise, 8 * cha, name='16')  # 16
        outs.append(r)

        x, r = g_block(x, inp_style[3], inp_noise, 6 * cha, name='32')  # 32
        outs.append(r)

        x, r = g_block(x, inp_style[4], inp_noise, 4 * cha, name='64')  # 64
        outs.append(r)

        x, r = g_block(x, inp_style[5], inp_noise, 2 * cha, name='128')  # 128
        outs.append(r)

        x, r = g_block(x, inp_style[6], inp_noise, 1 * cha, name='256')  # 256
        outs.append(r)

        x = tf.keras.layers.Add(name='generator_add')(outs)
        # x = tf.keras.layers.add(outs)

        x = tf.keras.layers.Lambda(lambda y: y / 2 + 0.5)(
            x)  # Use values centered around 0, but normalize to [0, 1],
        # providing better initialization

        self.G = tf.keras.Model(inputs=inp_style + [inp_noise], outputs=x)
    ...
    def GenModel(self):
        ...
        for i in range(n_layers):
            inp_style.append(tf.keras.layers.Input([latent_size]))
        ...
        self.GM = tf.keras.Model(inputs=inp_style + [inp_noise], outputs=gf)
    ...
    def GenModelA(self):
        ...
        self.GMA = tf.keras.Model(inputs=inp_style + [inp_noise], outputs=gf)
    ...
    def EMA(self):
        ...
        for i in range(len(self.G.layers)):
            ...
            for j in range(len(up_weight)):
                new_weight.append(old_weight[j] * self.beta +
                                  (1 - self.beta) * up_weight[j])
...
class StyleGAN(object):
    ...
    def evaluate(self, num=0, trunc=1.0):
        ...
        generated_images = self.GAN.GM.predict(n1 + [n2],
        ...
        generated_images = self.GAN.GMA.predict(n1 + [n2, trunc],
                                                batch_size=BATCH_SIZE)
        ...
        generated_images = self.GAN.GMA.predict(latent + [nImage(64), trunc],
                                                batch_size=BATCH_SIZE)
    ...
    def generateTruncated(self,
        ...
        if self.av.shape[0] == 44:  # 44 is an arbitrary value
            ...
            self.av = np.mean(self.GAN.S.predict(noise(2000), batch_size=64),
                              axis=0)
        ...
        generated_images = self.GAN.GE.predict(w_space + [noi],
                                               batch_size=BATCH_SIZE)
    ...
    def loadModel(self, name, num):
        ...
        mod = tf.keras.models.model_from_json(
            json, custom_objects={'Conv2DMod': Conv2DMod})
...
if __name__ == "__main__":
    ...
    elif sys.argv[1] in ('predict', 'gen'):
